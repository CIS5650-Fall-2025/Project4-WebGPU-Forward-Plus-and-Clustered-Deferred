...
export class Lights {
    ...
    constructor(camera: Camera) {
        ...
        this.numClusters =
            this.numClustersX * this.numClustersY * this.numClustersZ;

        const clusterByteSize = 4 + shaders.constants.maxLightsPerCluster * 4;
        this.clusterSetStorageBuffer = device.createBuffer({
            size: clusterByteSize * this.numClusters,
            usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
        });
    ...
    doLightClustering(encoder: GPUCommandEncoder) {
        ...
        computePass.dispatchWorkgroups(
            Math.ceil(this.numClusters / shaders.constants.clusterWorkgroupSize)
        );
        computePass.end();
    }
