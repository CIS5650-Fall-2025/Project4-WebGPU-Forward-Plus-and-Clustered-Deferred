import { Mat4, mat4, Vec2, Vec3, vec3, vec2 } from "wgpu-matrix";
...
class CameraUniforms {
    ...
    set nearFarPlane(nearFar: Vec2) {
        this.floatView[64] = nearFar[0];
        this.floatView[65] = nearFar[1];
    }

    set resolution(resolution: Vec2) {
        this.floatView[66] = resolution[0];
        this.floatView[67] = resolution[1];
    }
}
...
export class Camera {
    ...
    onFrame(deltaTime: number) {
        ...
        this.uniforms.nearFarPlane = vec2.create(Camera.nearPlane, Camera.farPlane);
